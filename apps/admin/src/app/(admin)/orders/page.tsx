...
export default function OrdersPage() {
  ...
  const { data, loading, error } = useApi(
    async () => unwrapMeta<Order>(await api.get('/admin/orders', { params: { search, status: status || undefined, limit: 50 } })),
    [search, status],
  );
    ...
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_420px]">
        ...
        <Card>
          ...
          <CardContent>
            {!selectedId ? <p className="text-sm text-muted-foreground">Select an order to inspect items and totals.</p> : null}
